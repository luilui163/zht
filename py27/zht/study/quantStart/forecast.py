...
def create_lagged_series(symbol,start_date,end_date,lags=5):
    ...
    of the adjusted closing value of a stock obtained from yahoo fanance,
    along with a number of lagged returns from the prior trading days
    (lags defaults to 5 days).
    Tradign volume,as well as the Direction from the previous day,are also
    included.
    '''
    ...
    for i,x in enumerate(tsret['Today']):
        if (abs(x)<0.0001):
            ...
            #this will result in some bias since the direction is determined
            #by the sign,this process will change those small minor value to
            #be positive
...
def fit_model(name,model,X_train,y_train,X_test,pred):
    '''
    Fits a classification model (for our purposes this is LR,
    LDA and QDA) using the training data,then makes a prediction and subsequent
    'hit rate' for the test data.
    '''
