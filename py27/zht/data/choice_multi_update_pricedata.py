...
def multi_save_df(tradedate,data, q):
    ths = []
    for i in range(100):
        th = threading.Thread(target=save_df, args=[tradedate,data, q])
        ths.append(th)
    for i in range(100):
        ths[i].start()
    for i in range(100):
...
def update_history_marketdata(startdate=None, enddate=None):
    database_path = get_database_path()
    c.start('rflh0000', '719181', "ForceLogin=1")
    if startdate == None:
        startdate = get_former_date()
    if enddate == None:
        enddate = time.strftime('%Y%m%d', time.localtime(time.time()))
    ...
    for tradedate in tradedates:
        ...
        for sn in stocknames:
            q.put(sn)
        multi_save_df(tradedate,data,q)
