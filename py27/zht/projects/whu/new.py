...
LOG_FILE = r'E:\whu\logfile\%s.log'%log_name
...
class Brute:
    def __init__(self,password_set,username_set,thread_num=50):
        self.password_list=password_set
        self.username_list=username_set
...
#            print('>'*20+'successfully!!!!!!!!!!!!'+username+'\t'+password)
...
#            print('wrong\t'+username+'\t'+password)
    ...
    def single_check(self):
        while not self.password_q.empty():
            self.lock.acquire()
            username=self.username_list[self.username_index]
    ...
    def multi_check(self):
        ...
        for _ in range(self.thread_num):
            t=threading.Thread(target=self.single_check)
            ts.append(t)
            t.start()
        for th in ts:
            th.join()
...
if __name__=='__main__':
    ...
    b=Brute(password_set,username_set,thread_num=5)
    b.multi_check()
#    b.multi_process()
