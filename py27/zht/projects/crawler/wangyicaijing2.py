...
#        href.append('http://quotes.money.163.com/old/#query=gn'+qid+'&DataType=HS_RANK&sort=PERCENT&order=desc&count=24&page=0')
        names.append(a[i]('a')[0].string.encode('gbk'))
        print names[-1],qids[-1]
    return names,qids

def get_data():
    date=time.strftime("%Y%m%d",time.localtime(time.time()))
    path=r'/dat/datadev/workspace/concept/wangyicaijing/%s'%date
    ...
    for q in range(len(qids)):
        ...
        stocks=[]
        while True:
            url='http://quotes.money.163.com/hs/service/diyrank.php?host=http://quotes.money.163.com/hs/service/diyrank.php&page=%d&query=PLATE_IDS:gn%s&fields=NO,SYMBOL,NAME,PRICE,PERCENT,UPDOWN,FIVE_MINUTE,OPEN,YESTCLOSE,HIGH,LOW,VOLUME,TURNOVER,HS,LB,WB,ZF,PE,MCAP,TCAP,MFSUM,MFRATIO.MFRATIO2,MFRATIO.MFRATIO10,SNAME,CODE,ANNOUNMT,UVSNEWS&sort=SYMBOL&order=desc&count=24&type=query'%(page,qids[q])
#            url='http://quotes.money.163.com/hs/service/marketradar_ajax.php?host=http://quotes.money.163.com/hs/service/marketradar_ajax.php&page=%d&query=PLATE_IDS:gn%s&types=&count=28&type=query&order=desc'%(page,qids[q])
            ...
            l=json.loads(content)['list']
            sid=[ll['SYMBOL'] for ll in l]
            #content=urllib2.urlopen(url).read()
