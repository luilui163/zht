...
def get_history_technicaldata():
    filedir1=r'C:\zht\OneDrive\script\rf\stock_monitor\technicaldata'
    loginResult = c.start('rflh0000', '719181', "ForceLogin=1")
    stocklist = [fn[:-4] for fn in get_filelist() if fn.endswith('.csv')]
    startdate='20090105'
    tradedates=get_tradedates(startdate)
    for stockname in stocklist:
        flag=1
        while flag:
            filepath = filedir1 + '\%s.csv' % stockname
            try:
                df = pd.read_csv(filepath, index_col=0)
                for i, d in enumerate(tradedates):
                    data = c.css(codes=stockname, indicators='MACD',TradaDate=d, options='Tradedate=%s' % d).Data[
                        stockname]
                    df.loc[d] = data
                    print stockname, d
                df.to_csv(filepath)
