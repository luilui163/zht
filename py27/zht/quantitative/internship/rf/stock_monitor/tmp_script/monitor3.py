...
def get_divergence_signal(df,smallest_total_bars):
    last_low_price=[0.0]*len(df.index)
    last_low_bar=[0.0]*len(df.index)
    last_high_price=[0.0]*len(df.index)
        ...
        if df['bar'].values[i]<0:
            ...
            while True:
                ...
                if df['bar'].values[i-r]<0 and df['bar'].values[i-r-1]>=0:
                    ...
                    break
                r+=1
            last_low_price[i]=get_last_low_value(m2,m1,'low')
            last_low_bar[i]=get_last_low_value(m2,m1,'bar')
            if df['low'].values[i]<=last_low_price[i] and df['bar'].values[i]>last_low_bar[i]:
            ...
            while True:
                ...
                if df['bar'].values[i-r]>=0 and df['bar'].values[i-r-1]<0:
                    ...
                    break
                r+=1
            last_high_price[i]=get_last_high_value(m2,m1,'High')
            last_high_bar[i]=get_last_high_value(m2,m1,'bar')
            if df['High'].values[i]>=last_high_price[i] and df['bar'].values[i]<last_high_bar[i]:
...
if __name__=='__main__':
    ...
    for stock_name in stock_names:
        df = datahandler_all.get_data(stock_name, 200)
