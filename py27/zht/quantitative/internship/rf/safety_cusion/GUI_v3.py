...
def initial_input_values():
    for k in input_dict:
        input_dict[k].delete(0,END)
        input_dict[k].insert(END,default_input_values[k])
...
def verify_input_values():
    for k in input_dict:
        ...
        try:
            ...
            tkinter.messagebox.showerror('data input error','the value format is invalid in %s'%k)
            return False
    return True
...
def calculate():
    ...
    if flag:
        ...
        clear_output_result()
        output_dict['total risk score'].insert(END,values.total_risk_score)
        output_dict['long term position'].insert(END,values.long_term_position)
        output_dict['breakeven loss range'].insert(END,values.breakeven_loss_range)
        output_dict['cash remained'].insert(END,values.cash_remained)
        output_dict['suggested cusion times'].insert(END,values.suggested_cusion_times)


def clear_output_result():
    for k in output_dict:
        output_dict[k].delete(0,END)
...
def config():
    ...
    for j,vo in enumerate(variables_output):
        ...
        output_dict[vo]=Entry(output_frame)
    ...
    Button(root, text='calculate', bg='blue', width=50, height=3, command=calculate) \
