...
def get_alpha(length):
    path=r'C:\cq\convertible_bond\data.txt'
    lines=open(path).read().split('\n')[1:-1]
    alpha=[]
    for l in lines:
        start_date=shift_date(l.split('\t')[2],0)
        end_date=shift_date(start_date,length)
        alpha.append((l.split('\t')[0],start_date,end_date,1))
        print l.split('\t')[0]
    return alpha
...
for length in [1,2,5,10,30,50]:
    alpha=get_alpha(length)
    get_result(alpha,r'C:\cq\convertible_bond\fig\%d.png'%length)
    print length