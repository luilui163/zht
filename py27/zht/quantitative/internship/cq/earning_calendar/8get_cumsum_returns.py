...
for length in range(1,21,2):
    for number in range(10,211,20):
        ...
        target_df.to_csv(r'C:\earning_calendar\early\target_df\%d_%d.csv'%(length,number))
        returns=target_df.mean(axis=1)
        returns=returns.fillna(0)
        cum_sum=returns.cumsum()
        cum_sum.to_csv(r'c:\earning_calendar\early\cumsum_returns\%d_%d.csv'%(length,number))
