...
def get_target_stocks_and_dates_of_one_quarter(path_of_quarter,days,trade_model):
...
def get_all_target_stocks_and_dates(days,trade_model):
    (target_stocks,target_dates)=get_target_stocks_and_dates_of_one_quarter(r'c:\earning_calendar\relative\Q1',days,trade_model)
    for q in range(2,5):
        path_of_quarter=r'c:\earning_calendar\relative\Q%d'%q
        (tmp_target_stocks,tmp_target_dates)=get_target_stocks_and_dates_of_one_quarter(path_of_quarter,days,trade_model)
        target_stocks.extend(tmp_target_stocks)
        target_dates.extend(tmp_target_dates)
    return(target_stocks,target_dates)

def get_target_df(stocks,dates,trade_model):
    date=[int(d) for d in dates]
    stock=stocks
...
def get_cumsum(short_target_df,long_target_df):
...
#    avg=returns.mean()
    ...
    fig.savefig(r'C:\earning_calendar\relative\combine\%d_%dcombine.png'%(length,days))
...
for days in range(1,10,2):
    for length in range(1,8,2):
        (short_target_stocks,short_target_dates)=get_all_target_stocks_and_dates(days,-1)
        (long_target_stocks,long_target_dates)=get_all_target_stocks_and_dates(days,1)
        short_target_df=get_target_df(short_target_stocks,short_target_dates,-1)
        long_target_df=get_target_df(long_target_stocks,long_target_dates,1)
        get_cumsum(short_target_df,long_target_df)
        print days,length
