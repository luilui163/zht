...
start=time.time()
p=r'C:\earning_calendar\quarter'
files=os.listdir(p)
filesPath=[os.path.join('%s\%s'%(p,f)) for f in files]

for length in range(1,21):
    for number in range(5,201,5):    
        early=open(r'C:\earning_calendar\result\early\early_length=%d_number=%d.txt'%(length,number),'w')
        late=open(r'C:\earning_calendar\result\late\late_length=%d_number=%d.txt'%(length,number),'w')
        ...
        #    path=r'c:\earning_calendar\quarter\2012Q4.txt'
            ...
            print path[-10:-4],'early',earlyafterReturn,earlybeforeReturn,earlyafterReturn-earlybeforeReturn
            print path[-10:-4],'late',lateafterReturn,latebeforeReturn,lateafterReturn-latebeforeReturn
            early.write('%s\t%f\t%f\t%f\n'%(path[-10:-4],earlyafterReturn,earlybeforeReturn,earlyafterReturn-earlybeforeReturn))
            late.write('%s\t%f\t%f\t%f\n'%(path[-10:-4],lateafterReturn,latebeforeReturn,lateafterReturn-latebeforeReturn))
        early.close()
        late.close()
end=time.time()
print end-start
