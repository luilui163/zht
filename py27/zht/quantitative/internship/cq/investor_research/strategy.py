...
lines=open(r'C:\cq\investor_research\data\data.txt').read().split('\n')[:-1]

industrys=[]
for l in lines:
    industry=l.split('\t')[4].split('、')
    for i in industry:
        if i not in industrys:
            industrys.append(i)

for length in [1,2,5,10,20,50]:
    for target_industry in industrys:
        alpha=[]
        for l in lines:
            industry=l.split('\t')[4].split('、')
            if target_industry in industry:
                date=l.split('\t')[0]
                stock=l.split('\t')[1]
                start_date=shift_date(date,1)
                end_date=shift_date(start_date,length)
                alpha.append((stock,start_date,end_date,1))
            
        figname=r'C:\cq\investor_research\eastmoneyIndustry_length\%s_%d.png'%(target_industry,length)
        position_df=alpha_to_position_df(alpha)
        get_result(position_df,figname)
        print length,target_industry
