...
wti=pd.read_csv(r'C:\cq\beta_exposure\oil\brent.csv',index_col=0)
...
#        corr.sort()        
        ...
        for i in range(stock_number):
            basket_low.append((dates[d],corr.index[i],corr.values[i]))
            basket_high.append((dates[d],corr.index[-(i+1)],corr.values[-(i+1)]))
            basket_alow.append((dates[d],m_corr.index[i],m_corr.values[i]))
        print dates[d]
...
#            xticklabels.append(str(month[i]))    
    ...
    fig.savefig(r'C:\cq\beta_exposure\oil\brent\%s.png'%fig_name)
