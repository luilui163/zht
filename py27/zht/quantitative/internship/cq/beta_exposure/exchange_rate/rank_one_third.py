...
def get_position_df(corr_window):
    corr_df=get_corr_df(corr_window)
    corr_df=corr_df.dropna(thresh=10)
    rank=corr_df.rank(axis=1)
...
#        rank.iloc[i,:]-=0.5
    ...
    for i in range(len(rank.index)):
        date=rank.index[i]
        position_df.loc[date,:]=rank.loc[date,:]
    return position_df
...
for corr_window in [10,20,30,50]:  
    position_df=get_position_df(corr_window)
    figname=r'C:\cq\beta_exposure\exchange_rate\ratio_on_third_reverse\%d.png'%corr_window
