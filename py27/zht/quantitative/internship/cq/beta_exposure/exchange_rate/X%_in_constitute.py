...
def get_basket(window,stock_number,constitute='HS300'):
    ZZ=open(r'C:\cq\beta_exposure\exchange_rate\ZZ500.txt').read().split('\n')
    HS=open(r'C:\cq\beta_exposure\exchange_rate\HS300.txt').read().split('\n')
    if constitute=='HS300':
        constitute=HS
    elif constitute=='ZZ500':
        constitute=ZZ
...
for window in [10,20,30,50]:
    for stock_number in [30]:
        for constitute in ['HS300','ZZ500']:
            basket_high,basket_low,basket_alow=get_basket(window,stock_number,constitute)
            for n,basket in enumerate((basket_high,basket_low,basket_alow)):
                basket_name=['high','low','alow']
                fig_name=r'c:\cq\beta_exposure\exchange_rate\constitute_corr\%s_%s_%d_%d.png'%(constitute,basket_name[n],window,stock_number)
                alpha=[]
                for b in basket:
                    start_date=shift_date(b[0])
                    end_date=start_date
                    alpha.append((b[1],start_date,end_date,abs(b[2])))
                get_result(alpha,fig_name)
                print constitute,window,stock_number,basket_name[n]
