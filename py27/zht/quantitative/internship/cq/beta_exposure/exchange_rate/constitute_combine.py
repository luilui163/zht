...
for stock_number in [5,10,20,30]:
    for constitute in ['HS300','ZZ500']:
        ...
        for bm in basket_alow:
            start_date=shift_date(bm[0])
            end_date=start_date
            alpha.append((bm[1],start_date,end_date,1))
        fig_name=r'C:\cq\beta_exposure\exchange_rate\constitute_combine\%s_%d_%d.png'%(constitute,window,stock_number)
        get_result(alpha,fig_name)
        print stock_number,constitute
