...
def get_basket(window):
    ...
    for d in range(window,len(dates)):
        ...
        corr.sort()
        stock_number=len(corr)
        basket_low+=[(end_date,corr.index[i],corr.values[i]) for i in range(int(stock_number/3))]
        basket_alow+=[(end_date,corr.index[i],corr.values[i]) for i in range(int(stock_number/3),int(stock_number*2/3))]
        basket_high+=[(end_date,corr.index[i],corr.values[i]) for i in range(int(stock_number*2/3),stock_number)]
        print dates[d]
    return basket_low,basket_alow,basket_high
...
def get_result(alpha,fig_name):
    ...
    fig.savefig(r'C:\cq\beta_exposure\exchange_rate\ratio\%s.png'%fig_name)


for window in [30]:
    basket_low,basket_alow,basket_high=get_basket(window)
    for n,basket in enumerate((basket_high,basket_low,basket_alow)):
        basket_name=['high','low','alow']
        fig_name=basket_name[n]+'_'+str(window)
        alpha=[]
        for b in basket:
