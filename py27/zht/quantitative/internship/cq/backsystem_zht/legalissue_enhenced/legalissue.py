...
data=[d for d in data if 20160812>int(d[1])]
...
'''
根据过去1000个样本的，数据，根据每个variable的value获得，avg return，作为权重。
'''
window=1000
thresh_number=100

alpha=[]
for d in range(1000,len(data)):
    window_data=data[d-1000:d]
    target_data=window_data[-1]
    stock=target_data[0]
    date=target_data[1]
    
    grades=get_grades(window_data,target_data,variables)
    final_grade=np.cumprod(grades)[-1]
    start_date=shift_date(date)
    end_date=start_date
    alpha.append((stock,start_date,end_date,final_grade))
