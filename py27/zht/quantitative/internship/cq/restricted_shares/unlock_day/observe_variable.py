...
def continuous_category(variable,hold_length=1):
        ...
        for d in data:
            ...
            try:
                if values[i+1]<float(d[variable.col])<values[i]:
                    ...
                    end_date=shift_date(start_date,hold_length-1) #notice that the parameter should be 0
...
def discrete_category(variable,hold_length=1):
    ...
    for i in range(len(values)):
        ...
        for d in data:
            if d[variable.col]==values[i]:
                stock=d[0]
                start_date=shift_date(d[1])
                end_date=shift_date(start_date,hold_length-1)
                category.alpha.append((stock,start_date,end_date,1))
...
def run():
    ...
    data=[d for d in data if 20100103<int(d[1])<20160812]
