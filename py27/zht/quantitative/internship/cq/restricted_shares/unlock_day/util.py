...
def alpha_to_position(alpha):
    ...
    for a in alpha:
        start_date=int(a[1])
        end_date=int(a[2])
        #the end_date is included
        position_df.loc[start_date:end_date,a[0]]=a[3]
        
    #deal with stock splitting and dividend
    valid_return_df=return_df.copy()
    valid_return_df[abs(valid_return_df)>0.105]=np.nan
    '''
    #in fact,we need not to remove the limit stock in the start_day ,becuase we invest it before the start_day
    '''
