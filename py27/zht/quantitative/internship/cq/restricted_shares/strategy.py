...
for length in [-100,-50,-30,-20,-10,-5,-3,-2,-1,0,1,2,5,10,20,30,50,100]:
    alpha=[]
    if length>=0:
        for item in items:
...
#            print n
    ...
    position=target_df.sum(axis=1)
    r=return_df.mean(axis=1)
    for i in range(len(position)):
        if position.values[i]<0:
            r.values[i]=r.values[i]*(-1)
    pnl=pd.Series([np.nan]*len(position),index=position.index)
    for i in range(len(position)):
        tmp_sum=(p.iloc[i]/abs(position.values[i])).sum()
        if tmp_sum!=0:
            pnl.values[i]=tmp_sum-r.values[i]
...
#    pnl=((return_df_limit*target_df).sum(axis=1))/abs(t)-r
    ...
    fig.savefig(r'C:\cq\restricted_share\contrast3\{length}.png'.format(length=length))
