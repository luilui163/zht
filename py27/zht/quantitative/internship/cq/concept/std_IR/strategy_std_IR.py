...
def get_stocks_list():
    index_return_df=pd.read_csv(r'c:\garbage\wind\adjust_index_return_df_l_h_5-50.csv',index_col=0)
    ...
    for di in range(len(dates)):
        for s in stocks_list[di]:
            if s in return_df.columns:
                if -0.09<return_df.at[dates[di],s]<0.09:
...
def fig():
    ...
    fig.savefig(r'C:\garbage\wind\ir_contrast\%d_%0.2f.png'%(ma,ratio))
