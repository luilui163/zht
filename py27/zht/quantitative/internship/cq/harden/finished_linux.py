...
def function(length,withdraw):
    ...
    (filePathList,fileNameList)=getAllFilePath('/home/hzhang/bloomberg')#注意一般情况下此处要将日期排序，由于此处读进来已经排好序，不必排了,可以用timeS=time.strptime(s,'%Y%m%d')转时间格式
    #判断排序是否正确
    ...
    pl.xlabel('date')
    pl.ylabel('return')
    pl.savefig('/home/hzhang/harden/test.png')
    pl.show()
#for zhanghaitao in range(26):
#    for cxt in np.arange(0.25):
