...
def get_liqSen():
    ...
    for i in range(1,len(monthStarts)):
        start=monthStarts[i]
        end=monthEnds[i]
        validStocks=_get_validStocks(monthEnds[i-1])
        #stocks with share prices less than $5 and greater than $1000
        #at the end of the previous month are filtered out.
    ...
    for j in range(1,len(months)):
        #months[j] denotes t,and months[j-1] denotes t-1
        tickers=delta.loc[months[j]].dropna().index.tolist()
        date=[m for m in monthEnds if m.startswith(months[j-1])][0]#the end of the month t-1
        if j==1:
        ...
        mts[months[j]]=mt
        deltaGammat[months[j]]=mt/m1*delta.loc[months[j]].mean()
        print months[j]
    ...
    lt=lt.fillna(method='ffill')#there are several NaNs,delete them
    lt.to_csv(os.path.join(idp,'liquidity.csv'))


    #================================================================
    #get the liquidity sensitivity for stocks
