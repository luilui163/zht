...
def get_indicatorId(name,param):
    ...
    if os.path.exists(p):
        ...
        ids=mathu.getSortedPortId(df,param)
        ids.to_csv(p)
        return ids
...
def get_portId(vars,model):
    ...
    if os.path.exists(p):
        ...
        if model=='2x3':
            ...
            id2=get_indicatorId(vars[1],[0.3,0.7])#TODO:ERROR,3 should be replaced with [0.3,0.7]
...
def get_factorRet_2x3():
    ...
    for var in ['btm','op','inv']:
        ...
        fr=fr.to_frame()
        fr.columns=[dd[var]]
        fr.to_csv(os.path.join(factorRetPath,'%s_%s.csv'%(model,dd[var])))
    ...
    SMB=SMB.to_frame()
    SMB.columns=['smb']
    SMB.to_csv(os.path.join(factorRetPath,'%s_%s.csv'%(model,dd['size'])))
...
#TODO: build camp and other models
