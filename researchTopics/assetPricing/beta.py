...
# TIME:2018-02-11  17:19
# NAME:assetPricing-beta.py
import numpy as np
import pandas as pd
from config import *
...
def usingDailyData():
    ...
    for length,thresh in dict1.items():
        ...
        for month in months:
            ...
            print(month)
        betadf.to_csv(os.path.join(BETA_PATH,length+'.csv'))
        r2df.to_csv(os.path.join(BETA_PATH,length+'_r2.csv'))
...
def usingMonthlyData():
    ...
    for length, thresh in dict2.items():
        ...
        betadf.to_csv(os.path.join(BETA_PATH, name + '.csv'))
        r2df.to_csv(os.path.join(BETA_PATH, name + '_r2.csv'))
