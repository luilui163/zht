...
def func(fns):
    for fn in fns:
        portId = pd.read_csv(os.path.join(myIndicatorIdPath, fn), index_col=0)
        ...
        for month in portId.columns.tolist():
            year = month[:4]
            validmonths=pd.date_range(start=str(int(year)+1)+'-07',end=str(int(year)+2)+'-07',freq='M')
...
if __name__=='__main__':
    fns = os.listdir(myIndicatorIdPath)
    multiNum=3
