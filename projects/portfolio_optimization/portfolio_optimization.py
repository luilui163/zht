...
'''
For details about the formulation and relevant documents, refer to the working directory of the project.
...
FACTORS=['log_size','bp','cumRet','turnover','amihud','ivol']# Review: do not include size
...
def get_factor_loading():
    ...
    for factor in FACTORS:
        if factor =='cumRet':
            ...
            s=pd.read_pickle(os.path.join(DIR_TMP,factor+'.pkl')).ffill().dropna(axis=0,how='all').stack()*(-1)  # trick:revert the directoin of amihud
        else:
...
def optimize0(N, F, r_f):
    ...
    w = Variable((N, 1)) #Trick: do not use nonneg,it seems like that the result can not satisfy the constraint when the solver is OSQP
    r_p = w.T @ (F.T @ r_f)
    obj = Maximize(r_p)
    constraints0=[
                sum(w)==1.0,
    ...
    # prob0.solve(solver=cvxpy.CVXOPT) # trick: the default solver can not get a solution satisfy the given constraint
    prob0.solve(solver=cvxpy.ECOS) # trick: the default solver can not get a solution satisfy the given constraint
...
def get_results(mode):
    ...
    factor_risk_cov=realized_factor_return.rolling(60).cov() #review: 60? # covariance rather than correlation
    factor_loading=pd.read_pickle(os.path.join(DIR_TMP,'factor_loading.pkl'))
...
def get_performance(mode):
    ...
    weight = weight.shift(1, freq='D').resample('D').pad()#trick: the weight is calculated at time t,and it is the target position of time t+1
